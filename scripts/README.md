...
### Harbor task image frontend build tools

For an OpenSandbox backend, setup prepares the pinned Go toolchain used by cold
frontend builds. `HARBOR_TASK_IMAGE_BUILD_TOOLS_SETUP=1` enables this explicitly;
...
the first HTTP(S) entry in `HARBOR_TASK_IMAGE_GOPROXY`. Other platforms must
provide the pinned version. A valid frontend cache does not require Go.

```bash
HARBOR_TASK_IMAGE_GOPROXY='https://goproxy.cn,direct' \
  HARBOR_TASK_IMAGE_BUILD_TOOLS_SETUP=1 bash scripts/setup.sh
```
