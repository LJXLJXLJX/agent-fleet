...
def compare(stock, patched):
    ...
    for path in files:
        ...
        for ex in after["execs"]:
            ex["meta"]["env"] = [
                (
                    "PATH="
                    if env == "PATH=/run/opensandbox-apt/bin"
                    else env.replace("PATH=/run/opensandbox-apt/bin:", "PATH=", 1)
                )
                if env.startswith("PATH=/run/opensandbox-apt/bin")
                else env
                for env in ex["meta"]["env"]
            ]
            extra = [
                mount
                for mount in ex["mounts"]
                if mount["dest"].startswith("/run/opensandbox-apt/")
            ]
    ...
    print(
        f"{len(files)} lowering differentials passed: all original ExecOp fields and image config preserved"
    )
