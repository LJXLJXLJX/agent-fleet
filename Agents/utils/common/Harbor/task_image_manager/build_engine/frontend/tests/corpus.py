...
def corpus():
    ...
    for (call_name, call), (shape_name, shape) in itertools.product(
        calls.items(), shapes.items()
    ):
    ...
    for name, argv in {
        ...
        "python": [
            "python3",
            "-c",
            "import subprocess; subprocess.run(['apt-get', 'probe'], check=True)",
        ],
    ...
    for interpreter, script in {
        ...
        cases["downloaded-" + interpreter] = (
            "RUN mkdir -p /tmp/http && printf %b "
            + shlex.quote(script.replace(chr(10), chr(92) + "n"))
