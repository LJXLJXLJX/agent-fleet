...
case "$command_name" in
    curl|wget) ;;
    *)
        echo "[harbor-task-image download] ERROR event=unsupported-command" >&2
        exit 126
        ;;
esac
...
if [ -z "$real" ]; then
    echo "[harbor-task-image download] ERROR event=real-binary-unavailable command=$command_name" >&2
    exit 127
fi
...
run_original() {
    reason=$1
    shift
    echo "[harbor-task-image download] event=download-bypass command=$command_name reason=$reason" >&2
    exec "$real" "$@"
}
...
if ! IFS= read -r source_root < "$runtime/source"; then
    echo "[harbor-task-image download] ERROR event=source-read-failed" >&2
    exit 125
fi
source_root=${source_root%/}
if [ -z "$source_root" ]; then
    echo "[harbor-task-image download] ERROR event=source-empty" >&2
    exit 125
fi
...
echo "[harbor-task-image download] event=download-cache-rewrite command=$command_name" >&2
...
if [ -z "$source_host" ]; then
    echo "[harbor-task-image download] ERROR event=source-host-empty" >&2
    exit 125
fi
