...
if [ ! -x "$real" ]; then
    echo "[harbor-task-image apt] ERROR event=real-binary-unavailable" \
        "command=$command_name path=$real" >&2
    exit 127
fi

if [ -n "${APT_CONFIG:-}" ]; then
    echo "[harbor-task-image apt] WARNING event=custom-source-config" \
...
for argument in "$@"; do
    ...
    case "${argument,,}" in
        *dir::etc=*|*dir::etc::sourcelist*|*dir::etc::sourceparts*|*rootdir*)
            echo "[harbor-task-image apt] WARNING event=custom-source-config" \
                "action=bypass argument=$argument" >&2
            exec "$real" "$@"
            ;;
...
shadow=$(mktemp -d "$runtime/shadow/invocation.XXXXXX") || {
    echo "[harbor-task-image apt] ERROR event=shadow-create-failed root=$runtime/shadow" >&2
    exit 125
}
...
if ! { printf '' > "$original_sources/sources.list" &&
    ...
    echo "[harbor-task-image apt] ERROR event=source-view-init-failed" >&2
    exit 125
fi
...
capture_source() {
    ...
    if ! cat "$original" > "$saved"; then
        echo "[harbor-task-image apt] ERROR event=source-copy-failed file=$display_file" >&2
        exit 125
    fi
    if ! LC_ALL=C awk -v gateway_file="$runtime/gateway-root" -v seen_file="$shadow/seen" \
        ...
        echo "[harbor-task-image apt] ERROR event=source-rewrite-failed file=$display_file" >&2
        exit 125
    fi
}
...
is_update=0
skip_option_value=0
quiet_value_may_follow=0
for argument in "$@"; do
    if [ "$skip_option_value" -eq 1 ]; then
        skip_option_value=0
        continue
    fi
    if [ "$quiet_value_may_follow" -eq 1 ]; then
        quiet_value_may_follow=0
        case "$argument" in
            ''|*[!0-9]*) ;;
            *) continue ;;
...
reconciliation_error() {
    echo "[harbor-task-image apt] ERROR event=index-reconciliation-failed phase=$1" >&2
    return 1
}
...
reconcile_indexes() {
    ...
    if [ -s "$shadow/rewritten-targets" ] && [ ! -s "$shadow/target-copies" ]; then
        reconciliation_error empty-targets
        return 1
    fi
