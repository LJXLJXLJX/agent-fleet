...
WRAPPER = (
    ...
    / "task_image_manager/build_engine/dockerfile_renderer/strategies/apt/apt-wrapper.sh"
)
...
class AptWrapperErrorsTest(unittest.TestCase):
        ...
        *,
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (sources / "sources.list").write_text(
                "deb http://origin.invalid stable main\n"
            )
            ...
            source = source.replace("/etc/apt", str(sources)).replace(
                "/usr/bin/", str(tools) + "/"
            )
            script.write_text(source)
            script.chmod(0o755)
            result = subprocess.run(
                [str(script), *(arguments or [command])],
                env={
                    "PATH": str(tools) + os.pathsep + os.defpath,
                    "CALLS": str(calls),
                    "APT_STATUS": str(apt_status),
    ...
    def test_preparation_errors_never_call_apt(self):
        for failure, event in [
            ("initialization", "source-view-init-failed"),
    ...
    def test_reconciliation_failure_is_not_hidden_by_conditional(self):
        ...
        self.assertEqual(result.returncode, 125, result.stderr)
        self.assertIn(
            "event=index-reconciliation-failed phase=join-targets", result.stderr
        )
        self.assertEqual(len(calls), 3)
    ...
    def test_unmatched_index_targets_fail_reconciliation(self):
        ...
        self.assertEqual(result.returncode, 125, result.stderr)
        self.assertIn(
            "event=index-reconciliation-failed phase=empty-targets", result.stderr
        )
        self.assertEqual(len(calls), 3)
