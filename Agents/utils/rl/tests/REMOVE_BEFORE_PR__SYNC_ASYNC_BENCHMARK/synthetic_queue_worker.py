    ...
    if args.delay_profile_json:
        ...
        for item in raw_profile:
            ...
            if fraction <= 0 or seconds < 0:
                raise ValueError("delay profile fractions must be positive and seconds non-negative")
            cumulative += fraction
...
def _claim_one(pending_path: Path, delay_seconds: float, sequence: int) -> ActiveExecution | None:
    try:
    ...
    return ActiveExecution(
        deadline=claimed_at + delay_seconds,
        sequence=sequence,
    ...
    while not STOP:
        ...
        if available > 0:
            ...
            for pending_path in pending_files[:available]:
                ...
                execution = _claim_one(pending_path, delay_seconds, sequence)
